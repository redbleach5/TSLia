...
function App(){
 ...
 useEffect(()=>{const s=socket.current;s.onOpen(()=>setConnected(true));s.onClose(()=>setConnected(false));s.onMessage((e:ServerEvent)=>{if(e.type==='state')setState(e.state);if(e.type==='transcript'&&e.text)setMessages(m=>[...m,{id:crypto.randomUUID(),role:'user',text:e.text,time:'сейчас'}]);if(e.type==='assistant_text')setMessages(m=>[...m,{id:crypto.randomUUID(),role:'assistant',text:e.text,time:'сейчас'}]);if(e.type==='audio')playAudio(e.data,e.mime);if(e.type==='error')setState('error')});s.connect();return()=>s.close()},[])
